...
export default class ModuleImportWriter {
  ...
  async writeImportDeclaration(filePath) {
    try {
      ...
      const npmNameList = await this.packageJsonFileManager.getInstalledNpmNameList();
  ...
  async removeImportDeclaration(filePath) {
    try {
      ...
      const postRemovedContent = removedFileLines.filter(prompt => !removedPrompt.includes(prompt));
